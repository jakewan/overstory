...
| `prTrajectory`| Per lookback window, pull requests opened, closed, and net — the change-request closure-ratio signal (`closed` counts merged and closed-without-merge). Aggregate (unaffected by `limit`); degradable. | `prtrajectory.go` |

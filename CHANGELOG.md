...
- `backlog_review` output is now a composite of named reduction blocks. Review-level identity (`repo`, `generatedAt`) stays at the top level; the staleness facts move under a `staleness` block, and the new deferred-issue facts arrive under a `deferred` block. Callers that read staleness fields at the top level must now read them under `staleness`. (#1)

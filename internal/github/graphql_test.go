...
func TestListIssueEventsDecodesPayloadPerType(t *testing.T) {
	...
	body := `[
		{"id":10,"event":"labeled","created_at":"2026-06-10T00:00:00Z","actor":{"login":"octocat"},
		 "issue":{"number":100,"title":"an issue","pull_request":null},"label":{"name":"reductions"},"performed_via_github_app":null},
		{"id":11,"event":"milestoned","created_at":"2026-06-10T01:00:00Z","actor":{"login":"octocat"},
		 "issue":{"number":100,"title":"an issue"},"milestone":{"title":"Round 6"}},
		{"id":12,"event":"assigned","created_at":"2026-06-10T02:00:00Z","actor":{"login":"octocat"},
		 "issue":{"number":100,"title":"an issue"},"assignee":{"login":"octocat"}},
		{"id":13,"event":"renamed","created_at":"2026-06-10T03:00:00Z","actor":{"login":"octocat"},
		 "issue":{"number":100,"title":"an issue"},"rename":{"from":"old","to":"new"},"performed_via_github_app":{"id":42}},
		{"id":14,"event":"closed","created_at":"2026-06-10T04:00:00Z","actor":{"login":"octocat"},
	...
	if e := byID[10]; e.Type != "labeled" || e.Label != "reductions" || e.Actor != "octocat" || e.IssueNumber != 100 || e.IssueIsPR || e.ViaAutomation {
		t.Errorf("labeled event = %+v", e)
	}
	if e := byID[11]; e.Milestone != "Round 6" {
		t.Errorf("milestoned.Milestone = %q, want Round 6", e.Milestone)
	}
	if e := byID[12]; e.Assignee != "octocat" {
		t.Errorf("assigned.Assignee = %q, want octocat", e.Assignee)
	}

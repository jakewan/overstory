...
From the `recommendations` block (its `candidates`). The server supplies per-issue candidate inputs and a neutral pre-sort; **the ranking judgment is this skill's** — the server reduces, the caller ranks. Each candidate carries `number`, `title`, `isBug`, `milestone` (the milestone *title*, or absent — there is no open/closed join, so a present `milestone` tells you only that the issue has one), `ageDays`, `inactiveDays`, and five dependency signals:
...
- **Confirmed ready** — *all* seams clear: `blockedBy` empty **and** `!blockedByTruncated`; `blockedByState` and `subIssueGapState` both **not** `unavailable` (an unread seam means the empty edge list and the zero gap are absences of evidence, not evidence of absence — treat any state other than `available` or `notApplicable` as unread, so a state added later fails safe here rather than promoting a candidate); the sub-issue gap zero (`subIssuesTotal − subIssuesCompleted == 0` — authoritative regardless of `subIssuesTruncated`, which bounds only the *named* children, and carrying no weight at all when `subIssueGapState` is `notApplicable`, since the forge has no children to count); and no `bodyRef` lands ∈ `openIssueSet.numbers`. Rank these first within the tier.
- **Indeterminate** — between the two, and where a candidate whose `blockedByState` or `subIssueGapState` is `unavailable` belongs: the backend could not read a seam its readiness rests on, so it is neither confirmed gated nor confirmed ready. Say which seam went unread rather than presenting the candidate as merely under-evidenced. Also here: a candidate carrying only `bodyRefs` that are **∉ `openIssueSet.numbers`** (not confirmed-open — possibly a closed issue, an open PR, a cross-repo ref, or, under `openIssueSet.fetchTruncated`, an open issue the window missed), or an empty edge list sitting under a true truncation flag. **Not** confirmed-gated (don't down-rank it as blocked) and **not** confirmed-ready (don't promote it) — surface the unresolved refs and order it after ready peers but ahead of confirmed-gated ones (a gated candidate is known-blocked; an indeterminate one only might be). On a candidate with non-empty `blockedBy` **and** `!blockedByTruncated`, its non-edge `bodyRefs` ∈ `openIssueSet.numbers` get the same treatment at the *ref* level: surfaced for verification alongside the candidate's edge-based gate, never counted as blockers (under `blockedByTruncated` they stay named blockers — see **Confirmed gated**).

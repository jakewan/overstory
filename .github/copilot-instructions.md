...
- **GitHub data is fetched in-process from the GraphQL API**, with credentials sourced from the operator's `gh` CLI (`gh auth token --hostname github.com`); `gh` is shelled out to only to fetch that credential. The token is requested for the host the requests go to — a token fetch that falls back to `gh`'s default host would send another host's credential to github.com, so flag one. Repo targeting is explicit (`owner/repo`); don't assume an ambient default repository.

...
- **GitHub data is fetched in-process from the GraphQL API**, with credentials sourced from the operator's `gh` CLI (`gh auth token --hostname github.com`) — so the server inherits existing `gh` authentication (no separate token to configure) without a subprocess per fetch while the token it holds is accepted. `gh` is shelled out to only to fetch that credential — whenever the server holds no token, and again when GitHub rejects the one it holds. The token is requested for the host the requests go to, never taken from `gh`'s default host (which `GH_HOST` or a lone Enterprise Server login can point elsewhere), and REST pagination refuses a link that leaves that host, so a login `gh` stores is only ever sent to the host that issued it. `GH_TOKEN`/`GITHUB_TOKEN` are the exception: `gh` applies them to github.com and `ghe.com` hosts alike, so an environment token is sent as set.

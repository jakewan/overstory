...
- **`gatesPrioritized`** — the subset of `blocking` whose target is a *prioritized* issue (milestoned or bug-labeled) in this window: the prioritized downstream work this candidate unblocks. It is the sharper form of raw `blocking` leverage — a ready candidate with a non-empty `gatesPrioritized` is a do-first gate root of a *priority*, not merely of arbitrary downstream work, so the server reserves a bounded number of such candidates a slot in the pre-sort (below) to keep the list cap from dropping the do-first blocker of a priority. Two seams floor it: `blockingTruncated` (the edge list was capped) **and** top-level `fetchTruncated` (a blocking target beyond the window can't be classified as prioritized, so it is silently excluded) — so under `fetchTruncated` an empty `gatesPrioritized` is a floor, not a confirmed "unblocks nothing prioritized," exactly as `bodyRefs ∉ numbers` is under `fetchTruncated`.

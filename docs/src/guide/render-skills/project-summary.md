...
The `seams` report states what the verdict rested on, giving each input `available`, `unavailable`, or `notApplicable`. `notApplicable` means this forge has no such relationship, so it is context rather than a gap — name it once ("readiness here rests on blocked-by edges alone") instead of flagging it per issue. `unavailable` means the input exists here and was not read, so every verdict in the block was reached without it: say so prominently, and read a large `provisionalCount` beside a zero `readyCount` as a backend that could not see the dependency data rather than a fully-gated backlog.

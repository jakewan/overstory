...
| `hygiene`         | Four signals over open issues: missing-area (qualified by `areaLabelsObserved` — false means the configured area taxonomy matched nothing in the window, so the count may reflect a repo that classifies nothing by area, one whose scheme is not declared, or an empty window), unmilestoned-and-aged, stale (neglected — deferred issues excluded), deferred-without-context. `labelTruncatedCount` floors all of them: a capped label list may have dropped the area or deferred label that decides a signal. | `hygiene.go` |

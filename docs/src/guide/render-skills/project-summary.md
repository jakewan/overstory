...
- **`bodyRefs`** — the distinct references to this repository's issues parsed from the issue body — its *stated* dependencies (a heuristic, not native edges). Parsed from the plaintext body, which keeps code spans and fenced blocks, so a reference quoted as code or in an example appears too; empty `bodyRefs` is not proof of no dependency. The plaintext body keeps only a markdown link's text, so `[#5](a URL in another repository)` arrives here as `5`: a ref ∈ numbers is a live issue in this repo, not proof the body meant that one. Resolve each against the top-level **`openIssueSet.numbers`**: a ref **∈ numbers** is a confirmed still-open issue in this repo (a live blocker **for a candidate without a recorded blocker list**; on a candidate with one, that list supersedes and a ref not in its `blockedBy` is demoted — see the recorded-blocker-list condition below); a ref **∉ numbers** is **unresolved/indeterminate** — it may be a closed issue, an open PR (PRs share the number space; `openIssueSet` is issues-only), or — under `openIssueSet.fetchTruncated` — an open issue the window missed. **Never read ∉ numbers as "resolved" or as "ready."**
...
Each band reads the server's `readiness` verdict and layers your own stated-dependency (`bodyRefs`) reading onto it — as a further condition on **Confirmed ready**, and as an additional route into the other two. Read the verdict; never re-derive it from the raw fields, which is how a rendering comes to disagree with the Dependency Classification in the same response.

One condition decides when a candidate's native edges supersede its `bodyRefs`, and every band below reads it: a candidate has a **recorded blocker list** when `blockedBy` or `blockedByExternal` is non-empty **and** `blockedByTruncated` is false. That list is the candidate's complete set of open blockers over recorded blocked-by edges, local and cross-repository alike; sub-issue children are not part of it. A candidate whose blocked-by lists are both empty has no recorded blocker list, however complete the emptiness, and neither does one whose list was capped.

- **Confirmed gated** — `readiness` is `blocked`, **or** a `bodyRef` ∈ `openIssueSet.numbers` on a candidate **without a recorded blocker list**. Present the candidate as *gated*, name its open blocker(s)/children — from `blockedBy`, from `blockedByExternal` (rendered `repo#number`), and from `subIssues` (where the sub-issue gap `subIssuesTotal − subIssuesCompleted` is positive but `subIssues[]` is empty, name "incomplete sub-issues" rather than open children, since the gap is an upper bound that can over-report), and rank it below ready peers in the same tier, so an aged capstone gated on open work no longer floats up on `ageDays` alone. **On a candidate with a recorded blocker list, that list is the authoritative open-blocker set** (with any sub-issue gap): a `bodyRef` not already in `blockedBy` is demoted to *indeterminate* (next bullet) — surfaced for verification, never named as a blocker — so an issue whose real dependencies are recorded as native edges is not mis-gated by a disclaimed prose `#N` ("no dependency on #N"). Under `blockedByTruncated` the edge list is a declared floor: a non-edge `bodyRef` ∈ numbers may be a truncated-out blocker, so keep it gating and named — do not demote.
- **Confirmed ready** — `readiness` is `ready` **and** no `bodyRef` lands ∈ `openIssueSet.numbers`. The verdict already carries every native seam: the edge list is complete rather than capped, and each seam was either read or does not exist on this forge — where `subIssueGapState` is `notApplicable` the gap counts carry no weight at all, since there are no children to count. Rank these first within the tier.
- **Indeterminate** — `readiness` is `provisional`: the edge list was capped, or the backend could not read a seam the verdict rests on, so the issue is neither confirmed gated nor confirmed ready. Check `blockedByState` and `subIssueGapState` to say *which* seam went unread rather than presenting the candidate as merely under-evidenced. Also here, on your side of the line: a candidate whose only `bodyRefs` are **∉ `openIssueSet.numbers`** (not confirmed-open — possibly a closed issue, an open PR, or, under `openIssueSet.fetchTruncated`, an open issue the window missed). **Not** confirmed-gated (don't down-rank it as blocked) and **not** confirmed-ready (don't promote it) — surface the unresolved refs and order it after ready peers but ahead of confirmed-gated ones (a gated candidate is known-blocked; an indeterminate one only might be). On a candidate with a recorded blocker list, its non-edge `bodyRefs` ∈ `openIssueSet.numbers` get the same treatment at the *ref* level: surfaced for verification alongside the candidate's edge-based gate, never counted as blockers (under `blockedByTruncated` they stay named blockers — see **Confirmed gated**).

**Within the confirmed-ready band, sequence by leverage.** A confirmed-ready candidate with a high `blocking` count is a do-first gate root — it stands in front of the most open downstream work — so rank it ahead of ready peers with little or no leverage, even when an aged peer is older. This is the per-candidate reflection of the `dependencies.gates` set (Step 2's Dependency Classification, ranked most-leverage-first): a ready candidate that also appears in `gates` is the clearest promote. **Sharper still: prefer `gatesPrioritized` leverage over raw `blocking` leverage** — a ready candidate that blocks *prioritized* work (a milestoned/bug issue named in `gatesPrioritized`) is the strongest do-first, since it directly advances a priority rather than arbitrary downstream work; rank it ahead of a ready peer whose leverage is only over non-prioritized issues. Honor the truncation seam — a `blocking` count under `blockingTruncated` is a floor (true leverage is at least that), so promote the candidate on its floor (render "blocks ≥N") above ready peers with lower or no leverage. The one caution: don't let a floored count leapfrog a peer whose *confirmed* (`!blockingTruncated`) `blocking` count already meets or exceeds that floor — there the truncated candidate's true leverage is genuinely unknown, so keep the confirmed-higher peer above rather than promote on an uncertain count. Leverage orders only *within* the ready band of a tier: it never lifts a ready candidate across a tier boundary, and never touches the gated/indeterminate ordering above.
...
When two candidates gate **each other** — mutual `blockedBy`, or mutual in-`openIssueSet.numbers` `bodyRefs` (a dependency cycle, common for sibling RFCs; the `bodyRefs` path applies only to candidates **without a recorded blocker list**, since a candidate with one has its non-edge refs demoted) — present them as a coupled gated cluster ranked as a unit below ready peers, rather than forcing a strict order between them. (The server already excludes the issue's own number and PR refs from `bodyRefs`, so the skill needs no self/PR filtering.) This re-sequencing is **independent of** the `listTruncated` note below — that flags the candidate *pool* was capped; the dependency seams flag *specific* gates. Both can fire.

```markdown
1. **#58 - Add ingest retry** (bug) — ready: no blockers, no open sub-issues, no live stated deps.
2. **#40 - Ingest schema** (aged backlog) — ready and high-leverage: `blocking`: blocks 3 open (#61, #71, #72); the do-first gate root, so it leads the ready aged work.
3. **#33 - Config loader cleanup** (aged backlog) — ready but low-leverage (blocks no open issue here); ranked below #40 despite being older — leverage orders the ready band.
4. **#47 - Refactor auth flow** (aged backlog) — stated dep #12 is *unresolved* (∉ openIssueSet.numbers — a closed issue or an open PR); indeterminate, not confirmed ready — verify #12 before starting.
5. **#72 - Ingest dashboard** (aged backlog) — *gated*: native `blockedBy` #40 (open, not truncated). Stated refs #18, #19 resolve **∈ openIssueSet.numbers** (confirmed-open) but are disclaimed prose mentions, not edges — demoted to verify-refs, not blockers, because its recorded blocker list is authoritative. Resume after #40.
```

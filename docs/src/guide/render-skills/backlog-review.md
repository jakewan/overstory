...
```markdown
- #63 - Title — readiness unconfirmed: blocked-by edges unread
- #64 - Title — readiness unconfirmed: blocked-by edge list capped
```

Name each issue's own cause from its own fields: `blockedByTruncated` means its edge list was capped, and `blockedByState` or `subIssueGapState` reading `unavailable` means that input was never read for this issue, so render it as unread rather than clear. More than one can hold; name each. Never take an issue's cause from `seams` — a seam reads `available` as soon as any fetched issue was read, so it says nothing about one issue. `seams` answers the repository-level question instead: a seam reading `unavailable` means no fetched issue yielded it — a backend that could not see the dependency data, not a backlog that is entirely gated — so name it once, prominently. If `provisionalTruncated` (the `limit` cap or a size trim), render `provisionalCount` as the total and say the unlisted issues' causes are not shown; don't infer them from `seams`.

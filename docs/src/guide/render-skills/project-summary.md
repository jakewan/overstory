...
- **`readiness`** — the server's verdict on whether the issue is actionable now: `ready`, `blocked`, or `provisional`. It is the same verdict, from the same decision, that Step 2's Dependency Classification counted this issue under, so the two never disagree within one response. `blocked` means an open blocker was observed (a native edge or open sub-issue children); `provisional` means none was observed but readiness could not be confirmed, because an edge list was capped or a seam went unread; `ready` means nothing open gates it and every input the verdict rests on was read or does not exist here. **It does not rank** — which candidate to do next is still this skill's judgment, and `readiness` carries no stated-dependency (`bodyRefs`) conjunct, which stays yours to apply below.

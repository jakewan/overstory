...
- **Scopes** (this project's areas): a code change scopes to the `internal/` package it centers on — the directory name (`manifest`, `reduce`, `backlog`, `summary`, `criticalpath`, … — illustrative, not a closed list, so a new package carries a scope without editing this rule), with two aliases where the scope name diverges from the directory: `mcp` for the server and tool contract (`internal/server`) and `gh` for the GitHub fetch layer (`internal/github`). A change spanning several packages scopes to its center of gravity — the package that owns it (e.g., a shared primitive consolidated into `internal/reduce` scopes `reduce`). Cross-cutting work that isn't a single package uses `ci`, `build`, `docs`, `rules`, `dx` (developer-experience: tooling, justfile, hooks), or `deps` (dependency updates, e.g. Dependabot bumps). A changelog-only commit — notably the post-PR-creation commit that anchors an entry to its PR number — belongs to no code area; scope it `docs` with no scope (`docs: ...`).

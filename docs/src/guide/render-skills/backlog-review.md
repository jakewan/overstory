...
**Provisional** is the unconfirmed-readiness class: an issue presenting no open blocker whose readiness the reduction could not confirm, so it is neither counted ready nor listed as a gate. Surface the count when non-zero. Two things land an issue here, and they warrant different wording. A capped `blockedBy` edge list (`blockedByTruncated`) is per-issue and usually rare — "readiness unconfirmed, edge list capped". A seam the backend could not read is structural: where it fires it typically fires across the whole window, so a large provisional count next to a zero ready count is a backend that could not see the dependency data, not a backlog that is entirely gated. Say which is in play rather than reporting a bare count.

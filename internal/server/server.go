...
// serverInstructions states what the server did to the dependency fields the two
// readiness tools project, so a caller cites the reduction rather than describing it in
// its own words or reading the sub-issue gap as an exact count. It is server-level
// rather than per-tool because both tools share it and the instructions are sent once
...
const serverInstructions = "Overstory returns reduced facts about a GitHub repository for the caller to render. " +
	...
	"(distinct, ordered by repository then number), and cross-repository blocking and sub-issue edges are dropped. " +
	"Each list is capped by the fetch: blockedByTruncated, blockingTruncated, and subIssuesTruncated mark a capped list as a floor, " +
	"and blockedByTruncated bounds blockedByExternal too. " +
	"subIssuesTotal − subIssuesCompleted comes from GitHub's summary over every child, in any repository, and is an upper bound on open children, not an exact count: " +
	"it is treated as a gate but can over-report after a not-planned closure, so a parent may read blocked with no open child, " +
	"and it is not a measure of how many children subIssues omits. " +
	"Derived fields: subIssueGate (backlog_review's dependencies block) is that gap read as a gate, false where the repository does not carry sub-issues; " +
	"gatesPrioritized (project_summary's recommendations) is the subset of blocking whose target is milestoned or bug-labeled within the fetched window, " +
	"a floor under blockingTruncated or fetchTruncated; " +
...
func New(opts ...Option) *mcp.Server {
	...
	srv := mcp.NewServer(&mcp.Implementation{Name: serverName, Title: serverTitle, Version: serverVersion}, &mcp.ServerOptions{Instructions: serverInstructions})
...
func backlogReviewTool() *mcp.Tool {
	return &mcp.Tool{
		Name:        "backlog_review",
		Description: "Grooming read of a GitHub repository's open-issue backlog as structured facts for the caller to render. Blocks: staleness (inactivity bands and the stalest issues, deferred ones excluded as parked); deferred (issues carrying the manifest's deferred labels, each with heuristic bodyRefs, native blockedBy/blocking/sub-issue edges, blockedByExternal for blockers in other repositories, named by repo and number, and a readiness verdict); areaBalance (spread across the manifest's areas); quality (thin body, no labels, or a missing required category); overlap (similar-title groups: candidate duplicates); crossRef (issues referencing one another: candidate consolidation); dependencies (convention-free ready/blocked/provisional counts, gates blocking open downstream work, and blocked issues with their blocked-by edges, the authoritative direction the crossRef mention graph can invert); trajectory (issues created, closed, and net per lookback window); prTrajectory (pull requests opened, closed including merged, and net per window); criticalPath (per declared stream, its open critical-path issues and whether its gate is cleared, provisional under a truncated fetch). Always returned: repo, generatedAt, and openIssueSet (the open issue numbers bodyRefs resolve against). Absence from openIssueSet is not proof of resolution: the ref may be closed, a pull request, in another repository, or beyond a truncated window. Readiness is the same verdict the dependencies block counts: blocked by an open blocked-by edge in any repository or an open sub-issue gate; provisional when no blocker was seen but a truncated edge list or an unread seam leaves it unconfirmed, since an empty edge list is not evidence of readiness. Everything covers the fetched window; truncation flags mark a floor. Blocks with their own fetch degrade to available:false. Under a size bound lists are trimmed and sizeBound says so. The blocks parameter returns a subset.",
...
func backlogReviewHandler(resolver *manifest.Resolver, fetcher github.Fetcher, now func() time.Time) mcp.ToolHandlerFor[backlogReviewInput, backlog.Facts] {
	return func(ctx context.Context, _ *mcp.CallToolRequest, in backlogReviewInput) (*mcp.CallToolResult, backlog.Facts, error) {
		...
		result.Issues = github.ApplyCapabilities(result.Issues, result.Capabilities)
		...
		dependencies := dependency.Reduce(result.Issues, result.TotalOpen, in.Limit, result.Capabilities)
...
// anything about the repository fetched: gh missing, gh holding no token GitHub
// accepts, or gh not answering. Every repository in a call shares the one token
// source, so a batch meeting one fails the whole call with it instead of marking each
// repository fetch_failed, and a tool whose only fetch would otherwise degrade to a
// marker fails the call too — either way the caller sees the fix. A caller's own
// deadline cutting a token fetch short is none of these.
func credentialFailure(err error) bool {
	return errors.Is(err, github.ErrGHNotFound) || errors.Is(err, github.ErrGHNotAuthed) || errors.Is(err, github.ErrGHTimedOut)
}
...
// zero items per repo rather than a repo-independent error. A credential failure is
// repo-independent, though, and fails the whole batch (see credentialFailure).
func maintenanceActivityBatchHandler(fetcher github.Fetcher, now func() time.Time, concurrency int, perRepoTimeout time.Duration) mcp.ToolHandlerFor[maintenanceActivityBatchInput, maintenance.BatchFacts] {
	return func(ctx context.Context, _ *mcp.CallToolRequest, in maintenanceActivityBatchInput) (*mcp.CallToolResult, maintenance.BatchFacts, error) {
		...
		entries, credErr := fanOutMaintenance(ctx, fetcher, repos, since, now, concurrency, perRepoTimeout)
		...
		if credErr != nil {
			return nil, maintenance.BatchFacts{}, credErr
		}
...
// fetch parameter; only `since` reaches the fetch (the floor it scans back to).
//
...
func fetchMaintenanceEntry(ctx context.Context, fetcher github.Fetcher, repo string, since time.Time, now func() time.Time, perRepoTimeout time.Duration) (maintenance.BatchEntry, error) {
	...
	if credentialFailure(err) {
		return maintenance.BatchEntry{Repo: repo, Unavailable: maintenance.UnavailableFetchFailed}, err
	}
	if errors.Is(err, github.ErrRepoNotFound) {
		return maintenance.BatchEntry{Repo: repo, Unavailable: maintenance.UnavailableNotFound}, nil
	}
	if rle, ok := errors.AsType[github.RateLimitedError](err); ok {

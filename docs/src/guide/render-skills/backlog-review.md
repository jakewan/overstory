...
The tool returns a composite object: `repo`, `generatedAt`, and one block per section — `deferred`, `quality`, `staleness`, `areaBalance`, `overlap`, `crossRef`, `trajectory`, `criticalPath`, `openIssueSet`, and an optional `rateLimit`. Render each section below from its block, in order, then synthesize Key Findings last. `openIssueSet` has no section of its own — it is the open-issue lookup the Deferred section resolves a parked issue's stated dependencies against.
...
Overstory reduces deferred issues to their parked-state facts (which labels, how long inactive, how old). It does **not** assess premise validity or missing rationale — those are judgment reads the old per-repo skills did by hand. `bodyRefs` surfaces an issue's stated `#N` dependencies when present; these are now *partially* resolvable against the `openIssueSet` block. A ref present in `openIssueSet.numbers` names a live open issue in this repo — so a parked issue still gated behind open work is distinguishable from one whose stated blockers are all gone. But **absence is not proof of resolution**: a ref missing from the set may be a closed issue (the blocker did clear), an open PR (PRs share the number space and aren't in this issues-only set), a cross-repo reference (a bare number), or — when `openIssueSet.fetchTruncated` — an open issue beyond the fetched window. Presence gates; absence only means "verify by hand." `bodyRefs` stays a heuristic proxy for *stated* cross-references, not GitHub's native blocked-by/sub-issue edges, and is itself a floor — parsed from GitHub's rendered plaintext, so a `#N` buried in a code fence won't appear. Don't fabricate the rest; if the user wants that depth, offer to read specific issue bodies during discussion.

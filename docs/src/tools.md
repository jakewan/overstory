...
For each open milestone, the parsed `tracks` in description order, each carrying a `label`, an optional raw `status` (a bold run-in's parenthetical, uninterpreted), and ordered `members` — each an issue `number` with a raw `statusToken` (`~~` for a struck/abandoned member, a checkbox marker char, or absent). Tracks are *started* by manifest-declared markers (heading levels and/or bold run-in labels) with a prose-section label stoplist; where a track *ends* is not configurable — every heading also bounds the track above it, following markdown's section nesting, so changing the marker levels never silently moves references between tracks. See [`milestoneTracks`](./manifest.md#milestonetracks). A description with no track structure yields a milestone with no tracks — the common case — not an error. The block is a single milestone fetch (which also retrieves each milestone's raw-markdown description), degradable as above; the milestone-list, per-milestone track-list, and per-track member-list truncation seams are each surfaced, alongside a per-milestone `unassignedRefs` count of references a boundary left outside every track.

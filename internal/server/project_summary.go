...
func projectSummaryTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Description: "Survey a GitHub repository for session orientation — \"given what's open now, what should I pick up?\" — and return compact structured facts for the caller to render: a milestones block (each open milestone's authoritative open/closed counts plus the fetched open issues belonging to it, with a per-milestone flag when that member list is a floor relative to the open count), an area-inventory block (per functional area, the active-vs-deferred split of its open issues, areas identified by the repo's manifest labels and prefixes), a hygiene block (four signals over the open issues: missing-area, unmilestoned-and-aged, stale, and deferred-without-context, plus an areaLabelsObserved flag reporting whether any fetched open issue carried a label the repo's configured area taxonomy recognizes — when false the count may reflect a repository that classifies nothing by area, or one classifying by a scheme the manifest does not declare, or simply an empty window, so a caller renders it as an observation to investigate rather than a list of defects to close; a labelTruncatedCount reports how many fetched issues had their own label list capped, which qualifies the three label-decided signals since a dropped tail may have held the area or deferred label — an error running in both directions rather than a floor), an open-PRs block (each open pull request's branch, draft/ready state, CI rollup, and inactivity, plus a stale-PR count), a recommendations block (per-issue inputs — bug-labeled, milestone, age, inactivity, and dependency signals: the heuristic bodyRefs plus the native blockedBy (and blockedByExternal, the open blockers in other repositories, each carrying the repository that qualifies its number), blocking, and open sub-issue children with their completion counts, and each candidate's readiness verdict of ready/blocked/provisional, which is the same decision the dependencies block below counted that issue under so the two cannot disagree within one response — a caller ranks 'what next' from; readiness is decided server-side because whether anything open gates an issue has one answer, while the ranking judgment stays caller-side), and a critical-path block (when the repo's manifest declares an ordered stream list and a critical-path label: each declared stream in order, its open critical-path-labeled issue members, and a per-stream gate-cleared signal — cleared meaning no open critical-path issue remains in the stream, provisional when the fetch window is truncated; absent the convention the block reports itself not configured), a dependencies block (open issues classified by their native blocked-by/blocking edges — convention-free: the ready/blocked/provisional counts and the gate set, the do-first roots that block open downstream work, each with how many downstream issues it unblocks. This is the graph-level classification only — the raw per-issue edges live in the recommendations block above; an issue is blocked by an open blocked-by edge — in this repository or another — or an open sub-issue gate, provisional when readiness cannot be confirmed — either a truncated edge list or a seam the backend could not read, since an empty edge list is not by itself evidence of readiness — and the classification is over the fetched window; a seams report states for each input the verdict rests on whether this repository carries it at all, so a seam marked notApplicable is why an issue can be ready with that input unexamined), and an open-issue-set block (the ascending, distinct set of open issue numbers in the fetched window — the resolvable surface for a candidate's stated bodyRefs, so a caller can tell a ref naming a live open issue in this repo from one that does not; same-repo, open, issues-only, and the full window never capped by limit, with a fetchTruncated flag marking when the set is a floor — presence names a live open issue to verify as a gate, absence is not proof of resolution, since the ref may be a closed issue, an open PR, a cross-repo reference, or beyond a truncated window). The milestones and open-PRs blocks each need their own fetch and mark themselves unavailable (with a rate_limited/fetch_failed reason) if that fetch fails, rather than failing the whole summary. The optional blocks parameter projects a subset: pass an allowlist of block names to return only those (omit it for the full composite), which also skips the secondary fetch backing any unrequested milestones/openPRs block; repo, generatedAt, and openIssueSet are always returned. " +
			dependencyFieldReductions +
			" A candidate's gatesPrioritized is the subset of its blocking whose target is milestoned or bug-labeled within the fetched window, and a gate's blockingCount is how many open downstream issues it unblocks; both are floors under blockingTruncated, and gatesPrioritized also under fetchTruncated.",

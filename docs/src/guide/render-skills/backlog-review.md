...
**The `seams` report says what the verdict rested on.** It gives each input a state: `available` (read), `unavailable` (exists on this forge, not read), `notApplicable` (this forge has no such relationship). A `notApplicable` seam is not a gap to flag — it is why an issue can be ready with that input unexamined, and the honest rendering names it once as context ("readiness here rests on blocked-by edges alone; this forge has no sub-issue hierarchy") rather than repeating it per issue or treating it as missing data. An `unavailable` seam is the opposite: name it prominently, because every verdict in the block was reached without it.

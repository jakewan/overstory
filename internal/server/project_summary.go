...
func projectSummaryTool() *mcp.Tool {
	return &mcp.Tool{
		...
		Description: "Orientation read of a GitHub repository, what to pick up next, as structured facts for the caller to render. Blocks: milestones (each open milestone's open/closed counts and fetched members, flagged when members are a floor); areaInventory (per area, the active-vs-deferred split); hygiene (missing-area, unmilestoned-and-aged, stale, and deferred-without-context counts; when areaLabelsObserved is false no configured area label was seen, so treat missing-area as an observation to investigate, not defects to close; labelTruncatedCount issues had labels capped, an error in either direction); openPRs (branch, draft state, CI rollup, and inactivity per open pull request); recommendations (per-issue inputs a caller ranks from: bug label, milestone, age, inactivity, heuristic bodyRefs, native blockedBy/blockedByExternal/blocking/sub-issue edges, and a readiness verdict); criticalPath (per declared stream, its open critical-path issues and whether its gate is cleared, provisional under a truncated fetch); dependencies (convention-free ready/blocked/provisional counts and gates with their blockingCount; per-issue edges live in recommendations). Always returned: repo, generatedAt, and openIssueSet (the open issue numbers bodyRefs resolve against). Absence from openIssueSet is not proof of resolution: the ref may be closed, a pull request, in another repository, or beyond a truncated window. Readiness is decided server-side, the same verdict the dependencies block counts; ranking stays with the caller. Blocked means an open blocked-by edge in any repository or an open sub-issue gate; provisional means no blocker was seen but a truncated edge list or an unread seam leaves it unconfirmed, since an empty edge list is not evidence of readiness. Everything covers the fetched window; truncation flags mark a floor. Blocks with their own fetch degrade to available:false. Under a size bound lists are trimmed and sizeBound says so. The blocks parameter returns a subset.",

...
- On Go 1.26+, prefer `errors.AsType[T](err)` over the pre-declared-var-plus-`errors.As` form when recovering a typed error: `if e, ok := errors.AsType[T](err); ok { … }`. It matches on the concrete dynamic type exactly as `errors.As(err, &target)` does, so the value-vs-pointer rule above still applies — `AsType[T]` needs `T`, not `*T`, in the chain.
...
- Result-set limits must be surfaced in the structured output, never silently truncated — a caller cannot tell incomplete data from complete data otherwise. This includes per-field truncation companions: when a reduction projects a fetched field that carries an upstream truncation flag (e.g. `github.Issue.BlockedBy` and `BlockedByTruncated`), carry the companion too — a list without its flag reads as complete. When a new block projects the same source fields as an existing one (`deferred`, `recommendations`, and `dependency` all project the native edge lists), match the sibling's truncation seams so the same field is never honest in one block and silently capped in another.
- **A change to observable output is a change to the docs that teach it.** When you add, remove, or change a reduction block, a documented field's meaning, a truncation contract, or the shape of a tool's output, review every doc surface that teaches that output, per the docs-pinning contract in `CONTRIBUTING.md`. Nothing at build time catches a doc that has fallen behind a reduction — the review is the guard.
...
- The github layer's wire-decode structs are drift-checked against their GraphQL query by `TestQueryDecodeContract`, which only inspects the structs its cases enumerate — but its walk descends into nested fields transitively. When you add a query or a new **root** decode type (a new query's connection, or a struct decoded straight from a data root), register it there — an unregistered root decode type is silently unguarded, the exact drift this test exists to catch. A struct nested inside an already-registered connection needs **no** separate case: adding a field to `issueNode` (reached transitively through the registered `issuesConnection`) is guarded automatically, so do not add a redundant registration for it. This guard is GraphQL-specific: it diffs a decode struct against its query string, so a **REST-sourced** decode struct (no query to diff against) cannot be registered there and is instead guarded by the opt-in live schema test (`TestLiveSchemaAcceptance`, run with `OVERSTORY_LIVE_REPO` set) — add a call exercising the new REST fetch there rather than reaching for the contract test.
...
- The repo-wide comment conventions — why-not-what, and comment durability — live in `.claude/rules/comment-conventions.md`. One Go-specific adjudication rides on top: durability asks you to name the identifier rather than its value, and in Go *which* identifier depends on what the value is — extract a Go constant when it is fetch mechanics (page sizes, byte budgets, protocol bounds); push it to the manifest when it encodes a repository's convention (labels, thresholds, formats), per § MCP server.

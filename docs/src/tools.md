...
| `hygiene`         | Four signals over open issues: missing-area (qualified by `areaLabelsObserved` — false means the repo classifies nothing by area, so the count is an observation, not a gap), unmilestoned-and-aged, stale (neglected — deferred issues excluded), deferred-without-context. | `hygiene.go` |

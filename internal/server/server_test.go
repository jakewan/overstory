...
// authored_activity, and authored_activity_batch tools. It is the end-to-end
// wiring proof; each tool's behavior is covered in its own _test.go.
func TestServerExposesTools(t *testing.T) {
	...
	for _, want := range []string{"backlog_review", "project_summary", "milestone_tracks", "authored_activity", "authored_activity_batch"} {
		if !got[want] {
			t.Errorf("tool %q not registered; got %v", want, got)
		}
	}
	if len(res.Tools) != 5 {
		t.Errorf("ListTools returned %d tools, want 5", len(res.Tools))
	}
}

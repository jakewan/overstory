...
Overstory does not manage credentials of its own. It authenticates to GitHub by shelling out to `gh auth token --hostname github.com` whenever it holds no token, and again whenever GitHub rejects the one it holds, inheriting the authentication the operator's [`gh`](https://cli.github.com/) CLI already holds for github.com. Consequences worth knowing:
...
- **The token is held in memory only** — fetched lazily on first use and cached until GitHub rejects it with HTTP 401, when the server asks `gh` again. Logging out of `gh` leaves the held token valid — `gh auth logout` removes the login only locally and does not revoke the token — so a running server keeps using it until it restarts or GitHub rejects it. Switching the active account with `gh auth switch` likewise leaves a running server on the token it holds, unless GitHub rejects that token; restart the server to switch accounts. Nothing is written to disk, and no token is embedded in a manifest or config file.
- **The token is never logged and never appears in a returned error.** When `gh` fails, the error is classified (not installed, did not respond in time, or no valid token could be obtained for github.com) without echoing the subprocess's stderr, which can carry sensitive detail.
...
- **The mise toolchain is reviewed manually.** No update bot covers `mise.toml`, so a second weekly workflow, **Toolchain currency**, checks the pins against upstream and **fails when any is behind — and equally when it could not check one**. Both halves matter: `mise outdated` exits 0 whether or not a pin has moved, and it drops a tool from its report rather than failing when an upstream lookup errors, so the run asserts that every declared pin was actually compared before reading a verdict. A green run therefore means checked and current, not merely unreported. How far the failure reaches is not a property of this repository, because Actions run notifications are a per-user setting. `CONTRIBUTING.md` records what GitHub does with them. It blocks nothing (neither workflow is a required check). `just toolchain-outdated` runs the same query locally. The pinned version of mise itself lives in the workflows rather than in `mise.toml`, so nothing reports it; it moves only when a maintainer moves it.
- **Scheduled scanning stops firing in a quiet repository.** [In a public repository, GitHub automatically disables scheduled workflows when no repository activity has occurred in 60 days](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows) — a condition on the repository, not on any one workflow. Read a gap in the weekly runs of either scheduled workflow as a symptom that the schedule stopped, not as an absence of findings.

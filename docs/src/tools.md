...
The server also sends MCP instructions when a session starts. They state what the server does to the dependency fields that `backlog_review` and `project_summary` return: the fields are reductions rather than GitHub's own lists, and the sub-issue gap is an upper bound on open children rather than a count. They are sent once per session because both tools share them. Claude Code shows a model only the first 2,048 characters of the instructions, the same limit it applies to each tool description, and the server's tests hold both to it. The text is `serverInstructions` in `internal/server/server.go`.
...
- **Degradation is per-block, not fatal.** Blocks needing their own fetch (`trajectory` in `backlog_review`; `milestones` and `openPRs` in `project_summary`; the whole of `milestone_tracks`, which is a single milestone-fetch reduction) carry `available`; when a fetch fails they set `available: false` and an `unavailable` reason (`rate_limited` or `fetch_failed`) instead of failing the whole call. A *hard* rate-limit failure on a tool's **primary** fetch — the open-issue fetch `backlog_review` and `project_summary` lead with — surfaces as a tool-call error rather than a degraded block. `milestone_tracks` has no primary fetch: its single milestone fetch degrades like the blocks above, so it never fails the call on a rate limit. Having no primary fetch to fail first, though, it does fail the call on a credential failure (`gh` not installed, holding no token GitHub accepts, or not responding), so the caller sees the fix rather than `fetch_failed`.
- **Total-size bound (`sizeBound`).** On a large repository the assembled response could exceed the MCP client's tool-result token cap and fail the call. To prevent that, the response trims its detail item-lists to fit the [`response.maxBytes`](./manifest.md#response) budget, balanced largest-contributor-first so no single block is gutted before the others. For the two composites this covers the block detail-lists — flat lists by item, overlap/cross-reference by whole group, milestone member-lists within each milestone (the milestone entries themselves are kept). `milestone_tracks` applies the same bound over its per-track member-lists, keeping every milestone and track headline. Counts, `openIssueSet`, the critical-path gate signal, and summary fields are never trimmed; a trimmed list sets its `listTruncated`, so under a bound a block's `limit` no longer predicts its listed count. A bounded response carries a top-level `sizeBound` (`maxBytes`, `finalBytes`, and per-block `{ block, dropped, remaining }`); `finalBytes` is one serialization (the wire carries roughly twice that) and is an upper bound, so when even the irreducible content exceeds the budget it reports the overflow rather than falsely claiming success. That overflow case is a real residual for `milestone_tracks`: its verbatim per-milestone descriptions are unbounded by design and cannot be trimmed, so on a prose-dominated milestone set its bound is best-effort — a partial bound over the member-lists, not the hard guarantee the two composites get. Struct: `reduce.SizeBoundFacts` in `internal/reduce/`.
- **`omitempty` fields.** `rateLimit` (top level) appears whenever a fetch reported a budget, which in practice is nearly every response, so its absence means no budget was observed rather than budget to spare. `sizeBound` appears only when the response had to be trimmed; `unavailable` appears only on a degraded block; a recommendation candidate's `milestone` is absent when the issue is unmilestoned. The projectable blocks (everything except the always-present `repo`, `generatedAt`, and `openIssueSet`) are likewise `omitempty`: present in a full-composite response, absent when [block projection](#block-projection-blocks) excludes them.
...
| `deferred`    | Open issues carrying the manifest's deferred labels (reports not-configured when none declared), each with its dependency edges and `readiness` verdict — the same decision the `dependencies` block counts it under. | `deferred.go` |
...
| `dependencies`| Open issues classified by their native blocked-by/blocking edges (convention-free): ready/blocked/provisional counts, a `gates` list (do-first roots that block open downstream work), and a `blocked` list carrying each issue's `blockedBy` edges (plus `blockedByExternal`, the open blockers in other repositories, each qualified by its repository since a bare foreign number would read as a local issue) — the authoritative direction the `crossRef` mention graph can invert. Provisional when readiness cannot be confirmed — a truncated edge list, or a seam the backend could not read, since an empty edge list is not by itself evidence of readiness. A `seams` report gives each input the verdict rests on one of `available`, `unavailable` (carried here, but no fetched issue yielded it) or `notApplicable` (this repository has no such relationship), so a caller can tell a verdict reached from every input from one reached from fewer. Classification over the fetched window. | `dependency.Facts` (in `internal/dependency/`) |
...
| `prTrajectory`| Per lookback window, pull requests opened, closed, and net — the change-request closure-ratio signal (`closed` counts merged and closed-without-merge). Returns the counts rather than a computed ratio. Aggregate (unaffected by `limit`); degradable. | `prtrajectory.go` |
| `criticalPath`| When the manifest declares a critical path: each declared stream in order, its open critical-path issue members, and a per-stream `gateCleared` signal; off-path/unareaed counts for misplaced issues. Sourced from the fetched window when it covers every open issue, else a dedicated critical-path-labeled fetch, so gate and members are authoritative regardless of backlog size — two signals mark a cleared gate provisional: `fetchTruncated` (rare) when the labeled subset itself exceeds the cap, and `labelTruncatedCount` when an on-path member's own labels were capped so its stream label may have been truncated out. `available: false` marks that fetch failing (the block degrades, the call does not). Not configured ⇒ `configured: false` no-op. | `criticalpath.Facts` (in `internal/criticalpath/`) |
...
| `hygiene`         | Four signals over open issues: missing-area (qualified by `areaLabelsObserved` — false means the configured area taxonomy matched nothing in the window, so the count may reflect a repo that classifies nothing by area, one whose scheme is not declared, or an empty window), unmilestoned-and-aged, stale (neglected — deferred issues excluded), deferred-without-context. `labelTruncatedCount` qualifies the three label-decided signals (not `unmilestoned-and-aged`): a capped label list may have dropped the area or deferred label that decides one, erring in either direction. | `hygiene.go` |
...
| `recommendations` | Per-issue inputs (bug-labeled, milestone, age, inactivity, native dependency edges) a caller ranks "what next" from — including which prioritized (milestoned/bug) work each candidate unblocks, so a caller can rank a ready blocker of a priority do-first, and each candidate's `readiness` verdict (`ready`/`blocked`/`provisional`), the same decision the `dependencies` block counted it under so the two cannot disagree within one response. The neutral pre-sort reserves those ready gates a slot so the list cap can't drop them; the ranking judgment stays caller-side. | `recommendations.go` |
| `criticalPath`    | When the manifest declares a critical path: each declared stream in order, its open critical-path issue members, and a per-stream `gateCleared` signal; off-path/unareaed counts for misplaced issues. Sourced from the fetched window when it covers every open issue, else a dedicated critical-path-labeled fetch, so gate and members are authoritative regardless of backlog size — two signals mark a cleared gate provisional: `fetchTruncated` (rare) when the labeled subset itself exceeds the cap, and `labelTruncatedCount` when an on-path member's own labels were capped so its stream label may have been truncated out. `available: false` marks that fetch failing (the block degrades, the call does not). Not configured ⇒ `configured: false` no-op. | `criticalpath.Facts` (in `internal/criticalpath/`) |
| `dependencies`    | The graph-level ready/blocked/gate classification (convention-free): counts, a `gateCount`, and the gate set with each gate's downstream `blockingCount` (a gate may not be the only blocker of the issues it counts). Provisional when readiness cannot be confirmed — a truncated edge list, or a seam the backend could not read. Carries the same `seams` report as `backlog_review`'s block (`available`/`unavailable`/`notApplicable` per input the verdict rests on). Classification-only — the raw per-issue edges live in `recommendations`, which carries each candidate's seam states and the per-issue `readiness` verdict behind these counts. | `dependency.Classification` (in `internal/dependency/`) |
...
For each open milestone, the parsed `tracks` in description order, each carrying a `label`, an optional raw `status` (a bold run-in's parenthetical, uninterpreted), and ordered `members` — each an issue `number` with a raw `statusToken` (`~~` for a struck/abandoned member, a checkbox marker char, or absent). Tracks are *started* by manifest-declared markers (heading levels and/or bold run-in labels) with a prose-section label stoplist; where a track *ends* is not configurable — a heading of equal or lesser depth than the track's own closes it while a deeper one is a sub-section whose references stay with it, a bold-run-in track taking the depth one level inside its enclosing section — so changing the marker levels can no longer move references across a sibling-or-shallower section boundary. See [`milestoneTracks`](./manifest.md#milestonetracks). A description with no track structure yields a milestone with no tracks — the common case — not an error. The block is a single milestone fetch (which also retrieves each milestone's raw-markdown description), degradable as above; the milestone-list, per-milestone track-list, and per-track member-list truncation seams are each surfaced, alongside a per-milestone `unassignedRefs` count of references a boundary left outside every track.

Plus the optional top-level `rateLimit` and `sizeBound`. The [`sizeBound`](#cross-cutting-conventions) trim here sheds only the per-track member-lists — the verbatim per-milestone description is unbounded by design and stays — so unlike the two composites the bound is best-effort: on a prose-dominated milestone set the description floor alone can exceed the budget and `finalBytes` honestly reports the overflow.
...
**Degradation is per repository, not all-or-nothing** — the one contract difference from the single-repo tool. A repository that is missing or failed degrades only its own entry while the rest return their counts (a throttle is the exception — it trips batch-wide backpressure, below). The aggregated `rateLimit` is the tightest remaining across the successful repositories, or — when any was throttled — `{ remaining: 0, resetAt }` at the earliest throttle reset, so a caller is never told it has budget mid-throttle. Three conditions still fail the whole call: a pre-fetch validation error (an empty or oversized `repos` list, a malformed or duplicate slug, an unparseable or inverted window); a **credential failure** — `gh` not installed, holding no token GitHub accepts, or not responding — which every repository shares and which surfaces as one error naming the fix; and an **unresolvable** `author` login — repo-independent, so it fails every repository identically and surfaces as one named error rather than N markers. When a batch meets both, the credential failure is the one reported, since resolving the login needs a working token.
...
**Degradation is per repository, and the fan-out adapts under load exactly as [`authored_activity_batch`](#authored_activity_batch) does** — a missing or failed repository degrades only its own entry; a `rate_limited` repository trips batch-wide backpressure so an arbitrary subset of the not-yet-started repositories returns `not_attempted`; each repository's fetch carries its own deadline. The aggregated `rateLimit` is the tightest remaining across the successful repositories, or `{ remaining: 0, resetAt }` at the earliest throttle reset. Pre-fetch validation (an empty or oversized `repos` list, a malformed or duplicate slug, an unparseable or inverted window) still fails the whole call, as does a credential failure, which every repository shares and which surfaces as one error naming the fix.

...
- Manifest resolution: per-repo manifests are discovered from `$XDG_CONFIG_HOME/overstory/manifests.d/*.yml` (and `*.yaml`), or from a colon-separated file list in `OVERSTORY_MANIFESTS` when set; entries are keyed by `owner/repo` and deep-merged over generic defaults. A repo's `owner/repo` key must be defined exactly once across the discovered files — a duplicate, whether across files or case-insensitively within one file, is a hard configuration error rather than a silent drop. (#4, #6)

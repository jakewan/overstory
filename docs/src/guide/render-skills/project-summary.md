...
Each band is the server's `readiness` verdict plus your own stated-dependency (`bodyRefs`) conjunct. Read the verdict; never re-derive it from the raw fields, which is how a rendering comes to disagree with the Dependency Classification in the same response.

...
- GitHub access: issues are fetched in-process via the GitHub GraphQL API, authenticated with the operator's existing `gh` credentials (`gh auth token`) — no separate token configuration. GitHub.com only for now.
- Manifest resolution: per-repo manifests are discovered from `$XDG_CONFIG_HOME/overstory/manifests.d/*.yml` (and `*.yaml`), or from a colon-separated file list in `OVERSTORY_MANIFESTS` when set; entries are keyed by `owner/repo` and deep-merged over generic defaults.

...
- **Confirmed gated** — `readiness` is `blocked`, **or** a `bodyRef` ∈ `openIssueSet.numbers` on a candidate with **no trustworthy-complete edge set** (`blockedBy` and `blockedByExternal` both empty, **or** `blockedByTruncated`). Present the candidate as *gated*, name its open blocker(s)/children — from `blockedBy`, from `blockedByExternal` (rendered `repo#number`), and from `subIssues` (where the sub-issue gap `subIssuesTotal − subIssuesCompleted` is positive but `subIssues[]` is empty, name "incomplete sub-issues" rather than open children, since the gap is an upper bound that can over-report), and rank it below ready peers in the same tier, so an aged capstone gated on open work no longer floats up on `ageDays` alone. **When `blockedBy` or `blockedByExternal` is non-empty *and* `!blockedByTruncated`, the two together are the authoritative open-blocker set — complete over recorded native edges** (with any sub-issue gap): a `bodyRef` not already in `blockedBy` is demoted to *indeterminate* (next bullet) — surfaced for verification, never named as a blocker — so an issue whose real dependencies are recorded as native edges is not mis-gated by a disclaimed prose `#N` ("no dependency on #N"). Under `blockedByTruncated` the edge list is a declared floor: a non-edge `bodyRef` ∈ numbers may be a truncated-out blocker, so keep it gating and named — do not demote.

...
| `labelStoplist` | `[]string` | common prose-section labels (`Why`, `Ikigai`, `History`, …) | Labels that are prose sections, not tracks (matched case-insensitively). A stoplisted label neither starts a track nor reports its references as unassigned when it only bounds one. Extend it for your repo's own section headings. |

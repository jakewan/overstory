...
- **`bodyRefs`** — the distinct references to this repository's issues parsed from the issue body — its *stated* dependencies (a heuristic, not native edges). Parsed from the plaintext body, which keeps code spans and fenced blocks, so a reference quoted as code or in an example appears too; empty `bodyRefs` is not proof of no dependency. Resolve each against the top-level **`openIssueSet.numbers`**: a ref **∈ numbers** is a confirmed still-open issue in this repo (a live blocker **for a candidate without a recorded blocker list**; on a candidate with one, that list supersedes and a ref not in its `blockedBy` is demoted — see the recorded-blocker-list condition below); a ref **∉ numbers** is **unresolved/indeterminate** — it may be a closed issue, an open PR (PRs share the number space; `openIssueSet` is issues-only), or — under `openIssueSet.fetchTruncated` — an open issue the window missed. **Never read ∉ numbers as "resolved" or as "ready."**
